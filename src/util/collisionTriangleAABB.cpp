// util/collisionTriangleAABB.cpp
// util/collisionTriangleAABB.cpp
// util/collisionTriangleAABB.cpp
//----------------------------------
 ...
 * by using the uncommented macro that follows, and a different test with it */
	...
	template<>
	inline float eps<float>() { return 10e-5f; }

	template<>
	inline double eps<double>() { return 10e-7; }
...
template<typename T>
struct Triangle3{
   Vector3<T> v1;                 /* Vertex1 */
   Vector3<T> v2;                 /* Vertex2 */
   Vector3<T> v3;                 /* Vertex3 */
}; 
...
template<typename T>
long face_plane(const Vector3<T> &p)
{
long outcode;
...
template<typename T>
long bevel_2d(const Vector3<T> &p)
{
long outcode;
...
template<typename T>
long bevel_3d( const Vector3<T> &p)
{
long outcode;
...
template<typename T>
long check_point(const Vector3<T> &p1, const Vector3<T> &p2, T alpha, long mask)
{
...
template<typename T>
long check_line(const Vector3<T> &p1, const Vector3<T> &p2, long outcode_diff)
{

   if ((0x01 & outcode_diff) != 0)
      if (check_point(p1,p2,( T(0.5)-p1.x)/(p2.x-p1.x),0x3e) == INSIDE) return(INSIDE);
   if ((0x02 & outcode_diff) != 0)
      if (check_point(p1,p2,(T(-0.5)-p1.x)/(p2.x-p1.x),0x3d) == INSIDE) return(INSIDE);
   if ((0x04 & outcode_diff) != 0) 
      if (check_point(p1,p2,( T(0.5)-p1.y)/(p2.y-p1.y),0x3b) == INSIDE) return(INSIDE);
   if ((0x08 & outcode_diff) != 0) 
      if (check_point(p1,p2,(T(-0.5)-p1.y)/(p2.y-p1.y),0x37) == INSIDE) return(INSIDE);
   if ((0x10 & outcode_diff) != 0) 
      if (check_point(p1,p2,( T(0.5)-p1.z)/(p2.z-p1.z),0x2f) == INSIDE) return(INSIDE);
   if ((0x20 & outcode_diff) != 0) 
      if (check_point(p1,p2,(T(-0.5)-p1.z)/(p2.z-p1.z),0x1f) == INSIDE) return(INSIDE);
   return(OUTSIDE);
}
...
template<typename T>
long point_triangle_intersection(const Vector3<T> &p, const Triangle3<T> &t)
{
	long sign12,sign23,sign31;
	Vector3<T> vect12,vect23,vect31,vect1h,vect2h,vect3h;
	Vector3<T> cross12_1p,cross23_2p,cross31_3p;
...
/**********************************************/
template<typename T>
bool t_c_intersection(const Triangle3<T> &t)
{
	long v1_test,v2_test,v3_test;
	T d, denom;
	Vector3<T> vect12,vect13,norm;
	Vector3<T> hitpp,hitpn,hitnp,hitnn;
...
{
	//Since our function assumes unit-sized aabb centered at world origin, we need to move the triangle around
	//and scale it properly.
	Vector3<T> vCubeCenter = (vCubeMins+vCubeMaxs)*T(0.5);
	Vector3<T> cubeSizeRezi = Vector3<T>(T(1.0)) / (vCubeMaxs - vCubeMins);

	Triangle3<T> t;
	t.v1 = (v0 - vCubeCenter) * cubeSizeRezi;
	t.v2 = (v1 - vCubeCenter) * cubeSizeRezi;
	t.v3 = (v2 - vCubeCenter) * cubeSizeRezi;

	return t_c_intersection<T>(t) == INSIDE;
}

//Explicit instantiation
template bool triangle_aabb_inters_t( const Vector3f &v0, const Vector3f &v1, const Vector3f &v2, const Vector3f &vCubeMins, const Vector3f &vCubeMaxs );
template bool triangle_aabb_inters_t( const Vector3d &v0, const Vector3d &v1, const Vector3d &v2, const Vector3d &vCubeMins, const Vector3d &vCubeMaxs );

template<typename T>
bool triangle_aabb_inters_t( Vector3<T> *pTriVerts, const Vector3<T> &vCubeMins, const Vector3<T> &vCubeMaxs )
{
	return triangle_aabb_inters_t<T>(pTriVerts[0], pTriVerts[1], pTriVerts[2], vCubeMins, vCubeMaxs);
}

//Explicit instantiation
template bool triangle_aabb_inters_t<float>( Vector3f *pTriVerts, const Vector3f &vCubeMins, const Vector3f &vCubeMaxs );
template bool triangle_aabb_inters_t<double>( Vector3d *pTriVerts, const Vector3d &vCubeMins, const Vector3d &vCubeMaxs );
...
{
	//TODO: optimize for 2D case
	float sizey = 1000000.0f;
	return triangle_aabb_inters_t<float>(triVertex1, triVertex2, triVertex3, Vector3f(aabbMin.x, -sizey, aabbMin.y), Vector3f(aabbMax.x, sizey, aabbMax.y));
}
